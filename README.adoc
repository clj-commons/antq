= Antq
:project-src-coords: clj-commons/antq
:project-mvn-coords: com.github.liquidz/antq
...
{workflow-url}/test.yml[image:{workflow-url}/test.yml/badge.svg[GitHub Actions for test workflow]]
{workflow-url}/lint.yml[image:{workflow-url}/lint.yml/badge.svg[GitHub Actions for lint workflow]]
{workflow-url}/dependencies.yml[image:{workflow-url}/dependencies.yml/badge.svg[GitHub Actions for dependencies workflow]]
...
https://clojurians.slack.com/archives/C0C4PCX9FJN[image:https://img.shields.io/badge/slack-join_chat-brightgreen.svg[Join chat]]
https://clojars.org/{project-mvn-coords}[image:https://img.shields.io/clojars/v/{project-mvn-coords}.svg[Clojars Project]]
https://github.com/users/clj-commons/packages/container/package/antq[image:https://img.shields.io/badge/docker-automated-blue["GitHub Container Registry"]]

[NOTE]
====
Antq requires Clojure 1.11 or later. +
Basically, antq can work regardless of the Clojure version on which your project depends. +
If you have trouble, please see link:./doc/non-supported-clojure-version.adoc[here].
====
...
| {sym-no}
| Experimental. Uses the project's `gradlew`, otherwise the `gradle` command must be installed.
...
* link:.github/workflows/dependencies.yml[.github/workflows/dependencies.yml]
...
For example, to <<opt-exclude>> artifact `a` and `b` you can express as: +
...
[TIP]
====
When running as a clojure tool, you might need some extra escaping for this option's value, .i.e.:
[source,shell]
----
clojure -Tantq outdated :error-format '"{{file}}::{{message}}"'
----
====
